...
class VideoWorkerThread(QThread):
    ...
    def run(self):
        ...
        while self.parent.thread_is_running:
            ...
            # TODO: Use self.createHOGDescriptor() to get human bounding box,
            # you may need to resize input frame for faster computation
            # Beware of the color space
            
            frame2 =cv2.resize(frame, dsize=None, fx=0.4, fy=0.4)
            frame =cv2.cvtColor(frame, cv2.COLOR_BGR2RGB)
            rects = self.createHOGDescriptor(frame2)
          
            # TODO: Draw the detections (rects) in the frame
            BOX_COLOR= (0, 0, 255)
            print(rects)
